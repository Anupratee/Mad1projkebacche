...
def register_influencer():
    if request.method == "POST":
        form_name = request.form.get("form_influencer_name")
        form_email = request.form.get("form_influencer_email")
        form_password1 = request.form.get("form_influencer_password1")
        form_password2 = request.form.get("form_influencer_password2")
        ...
        if form_password1 != form_password2:
            flash("Passwords must be the same", category="error")
            return redirect(url_for("auth.register_sponsor"))
        new_user = User(name = form_name, email = form_email, password = generate_password_hash(form_password1, method="pbkdf2:sha256"), role = "influencer")
        db.session.add(new_user)
        db.session.commit()
        flash("Account created successfully", category="success")
        return redirect(url_for("auth.login"))
